...
var scHPS = {
	...
	xAddBtn : function(pParent, pMgr, pFunc, pClassName, pCapt, pTitle, pNxtSib) {
		var vBtn = pParent.ownerDocument.createElement(scHPS.fButtonElement);
...
scHPS.PresMgr = function(pSldFraPath, pNavPath, pTocLnksPath, pIsMaster){
	...
	this.fListeners['onSldRestart'] = new Array();
...
scHPS.PresMgr.prototype = {
	...
	onLoad : function() {
		try{
			...
			//Init nav bar
			for (var i=0; i<this.fNav.length; i++){
				var vNav = this.fNav[i];
			...
			if (this.fAltSldRootPath && this.fAltSldFraPath){
				...
				var vAltSldFra = scPaLib.findNode(this.fAltSldRootPath)
				...
				var vAltSlds = scPaLib.findNodes(this.fSlidePath, vAltSldFra);
				for (var i=0;i<vAltSlds.length;i++){
					var vSld = vAltSlds[i];
					...
					for (var j=0;j<this.fSldRules.length;j++){
						try {
							var vRule = this.fSldRules[j];
							if (vRule.fFilter == "" || scPaLib.checkNode(vRule.fFilter, vSld)) vRule.fFunc(vSldMgr);
						} catch(e){scCoLib.log("WARNING PresMgr.onLoad - alternative slide rule num. " + j + ": "+e);}
					}
					vSld.fSldHdr.initSld();
					scHPS.xEndOpacityEffect(vSld, 0);
				}
				...
				if (vTocLnks.length>0){
					...
					for (var i=0; i < vTocLnks.length; i++){
						...
						if(vLnk.getAttribute("data-type") == "local"){
							var vLocalSlide = sc$(vLnk.hash.substring(1));
	...
	redrawSlideZone : function(){
		...
		for (var vSldId in this.fAltSlides.fSlides){
	...
	xUpdateSldPosStyledElts : function() {
		...
		var vSldClass = "";
		var vSld = this.fSwitchSldTask.fNewSld || this.fCurrSld;
		if (vSld && vSld.fSldHdr) {
		...
		if (this.fAltSlides && this.fAltSlides.fAct){
			...
			var vSldHdr = this.fAltSlides.fSldHdr;
			if (vSldHdr) {
		...
		for (var i=0;i<this.fSldPosStyledElts.length;i++){
			var vClass = this.fSldPosStyledElts[i].fBaseClass + vSldClass;
	 ...
	 * @param pIdx : integer - Slide index to load.
	 * @param pFromStart : boolean - Affiche le slide du début (true) ou de la fin (false).
	...
	xGotoSlide : function(pIdx, pFromStart, pInstantResult) {
		try{
			...
			var vFutureSld =  this.fSwitchSldTask.fNewSld;
			var vFutureIdx = vFutureSld ? vFutureSld.fSldHdr.fSldIdx : -9999;
			var vCurrIdx = this.fCurrSld ? this.fCurrSld.fSldHdr.fSldIdx : -9999;
			if(vCurrIdx == pIdx) {
	 ...
	 * @param pIdx : integer - slide index to build. */
	...
	xKeyMgr : function(pCharCode){
		...
		switch(vAction){
			...
			case "home":
				this.loadSlide(this.fFirstLocalIdx);return false;
			case "closeZoom":
	...
	xShowAltSlide : function(pRes, pOpt){
		var vZm = this.fAltSlides
		if(!vZm) return false;
		vZm.fSldHdr = null;
		var vPresMgr = this;
		var vSldId = pRes.hash.substring(1);
		if (vSldId.indexOf(":")>0){
			var vBlockId = vSldId.split(":")[1];
			vSldId = vSldId.split(":")[0];
		}
		var vSld = vZm.fSlides[vSldId];
		if (!vSld) return false;
		pOpt = pOpt || {};
		vZm.fSldHdr = vSld.fSldHdr;
		if (vZm.fCurrSld) new scHPS.FadeEltTask(vZm.fCurrSld,0,vPresMgr,null,null,true);
		new scHPS.FadeEltTask(vSld,1,vPresMgr,null,null,true);
	...
	xHideAltSlide : function(){
		if (this.fAltSlides && this.fAltSlides.fAct){
			...
			if (vZm.fCurrSld) new scHPS.FadeEltTask(vZm.fCurrSld,0,vPresMgr,null,null,true);
			vZm.fCurrSld = null;
			new scHPS.FadeEltTask(this.fAltSlides,0,this,scHPS.sFadeEltStart,scHPS.PresMgr.sFadeEltEnd);
...
scHPS.SldHdr.prototype = {
	...
	goToBlk : function(pBlk){
		if(pBlk) {
			var iHideAll =function(pHdr){
				var vSubs = pHdr.fBlks || pHdr.fSubBlocks
				if (vSubs){
...
scHPS.BlkCoHdr = function(pParentHdr, pBlkCoNode, pBlkIdx){
	...
	if (this.fAltBlksRoot) {
		...
		var vAltMenuItems = scPaLib.findNodes(this.fPresMgr.fCoAltBlocksMenuPathComp, this.fBlksRoot);
...
scHPS.SizeBlkObj = function(pElt, pOpt){
	...
	pElt.fObj = vObj;
	var vVideoElt = scPaLib.findNode("cde:video");
	if (vVideoElt){
...
scHPS.SizeBlkSvg = function(pElt, pOpt){
	...
	pElt.fSldMgr = pOpt.fSldMgr;
	pElt.fHdr = pOpt.fHdr;
	var vSvg = scPaLib.findNode(this.fPathSvg, pElt);
	if(!vSvg) {
		scCoLib.log("SizeBlkSvg - WARNING : cannot find base SVG element.");
		return;
	}
...
scHPS.SizeBlkSvg.prototype.onResizedAnc = function(pOwnerNode, pEvent){
	if(pEvent.phase==1) {
		try{
			...
			var vIsAlone = ! this.isEltNotAlone(pOwnerNode.fSldMgr.findOwnerBlk(pOwnerNode), pOwnerNode);
			var vRate = pOwnerNode.fSldMgr.getRatioNormalScreen() * pOwnerNode.fRatio;
			var vWantedW = vSvg.oriW * vRate;
			var vWantedH = vSvg.oriH * vRate;
			var vMaxW = pOwnerNode.fHdr.getAvailWidth() *  (this.fOpt.ratioWidth || .8);
			var vMaxH = pOwnerNode.fHdr.getAvailHeight() * ( vIsAlone ? (this.fOpt.ratioHeightAlone || .9) : (this.fOpt.ratioHeight || .8) ) - (this.fOpt.captionHeight || 30);
			pOwnerNode.fZoomOn = false;
			if(vWantedW > vMaxW || vWantedH > vMaxH) {
...
scHPS.SizeBlkSvg.sOnClickZoom = function(pEvent) {
	...
	if (!vBlk.fSldMgr || !vBlk.fSldMgr.fSldHdr) return false;
	var vSldHdr = vBlk.fSldMgr.fSldHdr;
	var vZmItm = vSldHdr.showZoom(vBlk.fSvg);
	var vCt = vSldHdr.getZoomContainer();
	var vResizer = {
		onResizedDes : function(pOwnerNode, pEvent) {},
		onResizedAnc : function(pOwnerNode, pEvent) {
			if(pEvent && pEvent.phase==2) return;
			var vRate = vBlk.fSldMgr.getRatioNormalScreen() * vBlk.fZoomRatio;
			var vWantedW = vBlk.fSvg.oriW * vRate;
			var vWantedH = vBlk.fSvg.oriH * vRate;
			var vMaxW = vCt.offsetWidth;
			var vMaxH = vCt.offsetHeight;
			if(vWantedW > vMaxW || vWantedH > vMaxH) {
				var vRW = vWantedW / vMaxW;
